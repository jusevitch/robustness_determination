function outstruct = rRoblower(args)

% Finds a lower bound on the robustness of undirected graphs (directed
% too?) by checking only the sets with |S| <= floor(n/2)
...
if not_solved_before_MaxTime_bool == 0
    ...
    if isfield(args,'print') && args.print == 1
        ...
        disp(['Computation time: ' time])
        
        disp('One possible minimal S:')
        outstruct.b'
    end
