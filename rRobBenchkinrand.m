function outstruct = rRobBenchkinrand(args)
...
% the maximal r-robustness of k-in random digraphs.
% Analogous to k-out random graphs in Bollobas 2001, "Random Graphs", 
% Chapter 2.
%
...
% args.repeatkinrand : number of times to repeat the benchmark for each n and p
...
addAttachedFiles(pool,{'../kinRandDigraph.m'}); % Change to specify path to ErdRen.m file if necessary
...
if isfield(args,'repeatkinrand') && args.repeatkinrand > 1
    ...
    % by args.repeat
    totveck = repmat(totveck,args.repeat,1);
    totveck = sortrows(totveck,[1 2]);
end


parfor ii=1:1:length(totveck)
    ...
    if algs(1) == 1
        ...
        disp([newline 'Completed k-in LeBlanc algorithm for n=' num2str(n) ' and k=' num2str(k) newline]); % char(10) makes a new line
...
disp(['k-in RD Graphs analysis done' newline])
